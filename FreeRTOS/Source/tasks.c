...
// [ILG] - begin
  ...
  // This is a generic version of Resume, based on vTaskResume
  ...
  TCB_t * const pxTCB = ( TCB_t * ) xTaskToResume;
    ...
    if( ( pxTCB != NULL ) && ( pxTCB != pxCurrentTCB ) )
    {
      taskENTER_CRITICAL();
      {
        // [ILG]
        // if( prvTaskIsTaskSuspended( pxTCB ) != pdFALSE )
        if( 1 || prvTaskIsTaskSuspended( pxTCB ) != pdFALSE )
        {
          traceTASK_RESUME( pxTCB );
  ...
  // These two are a split of vTaskSuspend(),
  // with xTaskToSuspend replaced by pxCurrentTCB.
